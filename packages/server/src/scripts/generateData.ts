...
const user2 = await usersSvc.create({ username: "john_doe", email: "john.doe@example.com" });
const user3 = await usersSvc.create({ username: "jane_smith", email: "jane.smith@example.com" });
...
await transactionsSvc.create({
  user: user2._id as Schema.Types.ObjectId,
...
await transactionsSvc.create({
  user: user3._id as Schema.Types.ObjectId,
